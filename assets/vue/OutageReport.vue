...
<template>
  <div class="card shadow-sm h-full flex flex-col">
    <DataTable v-model:filters="filters" v-model:selection="selectedReports" v-model:editingRows="editingRows"
      :value="searchableReports" :globalFilterFields="globalFields" dataKey="id" ref="dt" editMode="row"
      @row-edit-init="onRowEditInit" @row-edit-save="onRowEditSave" scrollable scrollHeight="flex"
      :virtualScrollerOptions="{ itemSize: 50 }" selectionMode="multiple" stripedRows removableSort sortMode="multiple"
      resizableColumns showGridLines rowHover class="flex-1 cursor-pointer text-sm">
      <template #header>
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-4 py-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <Button label="Export CSV" @click="exportCSV" size="small"
      ...
      <Column field="status" sortable header="Status">
        ...
        <template #editor>
          <div class="flex flex-col gap-1">
            <Select v-model="statusField.value.value" :options="reportStatusOptions" optionLabel="label"
              optionValue="value" :invalid="statusField.isTouched.value && !!statusField.errorMessage.value"
              @blur="statusField.blur()" fluid />
